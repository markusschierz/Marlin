...
#ifdef QQSP
  #define CUSTOM_MACHINE_NAME "Markus QQS-Pro"
#endif
#ifdef Q5
...
#define PID_K1 0.95      // Smoothing factor within any PID loop
  ...
  //#define PID_PARAMS_PER_HOTEND // Uses separate PID parameters for each extruder (useful for mismatched extruders)
    ...
    #define DEFAULT_Kp_LIST {  18.94,  18.94 }
    #define DEFAULT_Ki_LIST {   1.25,   1.25 }
    #define DEFAULT_Kd_LIST {  71.75,  71.75 }
  #else
    #ifdef Q5
...
#define DELTA
  ...
  // NOTE NB all values for DELTA_* values MUST be floating point, so always have a decimal point in them
    ...
    #define DELTA_RADIUS 141.26               // (mm) Get this value from G33 auto calibrate

    // Distance between bed and nozzle Z home position
    #define DELTA_HEIGHT 355.19                //370 E3D-360 (mm) Get this value from G33 auto calibrate

    #define DELTA_ENDSTOP_ADJ { -0.87, -1.71, 0.0 } // Get these values from G33 auto calibrate

    // Trim adjustments for individual towers
    // tower angle corrections for X and Y tower / rotate XYZ so Z tower angle = 0
    // measured in degrees anticlockwise looking from above the printer
    #define DELTA_TOWER_ANGLE_TRIM { 0.17, -0.11, -0.05 } // Get these values from G33 auto calibrate
